...
발표 시간이 짧다면 6단계(★)만 다뤄도 됨. 각 단계는 "지금까지 설명한 걸로 이게 가능해진다"는
식으로 앞 단계 위에 쌓이게 짜여있어서, 순서를 건너뛰면 다음 단계 설명이 붕 뜰 수 있음.

1. **★ 전체 구조 개요** — 위 블록 다이어그램으로 시작. "버튼도 읽어야 하고, 신호도 재야
   하고, 화면에도 그려야 한다" 정도의 큰 그림 + SPI 버스 공유 제약(가장 눈에 띄는 설계
   포인트라 초반에 던져두면 뒤에 나오는 poll 콜백 구조가 자연스럽게 이해됨).

2. **★ main.c → app.c** — 프로그램이 어떻게 시작해서 어떻게 도는지(`main()`의 초기화
   순서, `app_run_frame()`의 한 바퀴 흐름: 입력폴링 → 그리기 → 지연 반영). "이 프로젝트의
   코드 진입점과 전체 제어 흐름"을 여기서 확정하고 시작하면 이후 각 모듈이 이 흐름의
   어디에 끼워지는지 설명하기 쉬움.

3. **hc165.c → input.c → ui.c** — 버튼/엔코더가 어떻게 16비트 값 하나로 읽혀서(hc165),
   디바운스·엣지·쿼드러처 디코딩을 거쳐(input), UI 상태머신(ui)으로 반영되는지. 입력
   체인은 독립적이고 이해하기 쉬워서 초반에 처리하면 좋음.

4. **★ adc.c** — 이 프로젝트의 기술적 핵심. ADC 1개로 2채널을 MUX 전환하며 번갈아 재는
   구조, 타이밍 제약(변환시간+안정화딜레이 vs 샘플레이트), 핑퐁 버퍼링으로 ISR-메인루프
   레이스를 막은 것, 자동 트리거(제로크로싱) 탐색. 발표에서 가장 공들여 설명할 가치가
   있는 부분 — "왜 이렇게 짰는지"에 대한 이유(하드웨어 제약)가 명확해서 질문 대응하기도
   좋음.

5. **calib.c** — adc.c가 만든 raw값을 실제 전압으로 바꾸는 보정 과정(2점 캘리브레이션,
   EEPROM 저장). adc.c 바로 다음에 놓으면 "raw → mV" 흐름이 끊기지 않음.

6. **★ grid.c → waveform.c** — 격자/화면분할 영역 정의부터 시작해서, 파형을 부분갱신
   방식으로 그리는 방법(깜빡임 없는 이유)까지. 실제 동작 화면(사진/영상)을 같이 보여주면
   효과적인 파트.

7. **fft.c → spectrum.c** — FFT 스펙트럼 그래프(보너스 기능). 고정소수점 FFT를 AVR에서
   직접 구현했다는 것 자체가 어필 포인트. 6번(파형 그리기)을 이해한 뒤라 "같은 파이프라인,
   다른 렌더러"로 설명하면 빠르게 넘어갈 수 있음.

8. **★ statusbar.c** — RMS/MAX/MIN/FREQ/PERIOD 계산과 표시. adc.c(샘플), calib.c(mV
   환산), grid.c(화면 배치)가 여기서 다 합쳐지는 지점이라 "지금까지 만든 걸 실제 측정값
   으로 보여주는 단계"로 마무리하기 좋음.

9. **display.c → text.c → spi.c → millis.c** — 가장 아래 계층(LCD 드라이버, 폰트, SPI,
   ms 타이머). 다른 모든 모듈이 이걸 딛고 서 있지만, 내용 자체는 레지스터 설정이라
   발표에선 "이런 저수준 드라이버들도 직접 다 짰다" 정도로 짧게 언급하고 넘어가도 충분함.
   시간이 없으면 통째로 생략 가능.
...
**핵심 설계 포인트**: 이 프로젝트는 소프트웨어 프레임버퍼를 전혀 두지 않는다. ILI9488이
자체 GRAM(프레임버퍼)을 내장하고 있어서, MCU는 `lcd_set_window()`로 영역만 지정하고
SPI로 픽셀값을 스트리밍하면 화면 유지는 컨트롤러가 전담한다. 만약 480×320×3바이트
(≈460KB) 프레임버퍼를 ATmega128 SRAM(4KB)에 직접 둬야 했다면 이 프로젝트는 애초에
불가능했다 — GRAM 덕분에 SRAM은 오직 캡처 버퍼·파형 상태처럼 "지금 그릴 값"만 담으면
된다.
