...
void adc_init(void)
{
    ...
    // 데이터시트 권장(≤200kHz)보다 빠르지만, 10bit 정밀 측정보다 파형 모양을 빠르게
    // 그리는 게 더 중요해서 속도 쪽으로 택함 - 변환 1회 약 26us, CH1+CH2 한 쌍 약 52us.
...
void adc_set_time_div(uint8_t time_div_idx)
{
    ...
    if (rate < 1)
        rate = 1;
    // CH1+CH2 한 쌍 변환 최소 52us + MUX 전환 안정화 딜레이 10us + 인터럽트 오버헤드를
    // 더하면 실제로는 ~67~70us 필요함 - 그보다 빠른 레이트를 요청하면 타이머가 다음
    // 변환을 걸 때 이전 변환이 아직 안 끝나 샘플이 꼬인다. 12000Hz(83.3us)로 클램프해서
    // 여유를 둔다.
    if (rate > 12000UL)
        rate = 12000UL;
...
uint16_t adc_find_trigger(const volatile uint16_t *capture_buf_ch1)
{
    ...
    // 신호가 한쪽으로 치우쳐 0V를 안 지나가면(RC 충방전 곡선 등) 고정 0V 기준으론
    // 크로싱을 영영 못 찾아 폴백만 반복되고, 캡처 경계가 신호 위상과 안 맞아 화면이
    // 계속 흘러가 보인다. statusbar.c의 find_period_us()와 같은 이유, 같은 해법.
...
ISR(ADC_vect)
{
    ...
    if (adc_active_ch == 0) {
        ...
        // 시간을 줌(10us, 83.3us 예산 안에서 충분히 여유있음).
